let zombie2SpriteSheet = new Image("graphics/zombies/zombie2_spritesheet.png");
...
export default class Zombie2 {
    ...
    update(defenders) {
      ...
      if (this.frameCounter >= this.frameSpeed) {
        this.frameCounter = 0;
        this.currentFrame = (this.currentFrame + 1) % this.leftFrames2.length;
      }
    }
    ...
    draw() {
        if (zombie2SpriteSheet && zombie2SpriteSheet.ready() && this.leftFrames2.length > 0) {
            const frame = this.leftFrames2[this.currentFrame];

            if (frame) {
                zombie2SpriteSheet.startx = frame.x;
                zombie2SpriteSheet.starty = frame.y;
                zombie2SpriteSheet.width = frame.width;
                zombie2SpriteSheet.height = frame.height;
                zombie2SpriteSheet.endx = frame.endx;
                zombie2SpriteSheet.endy = frame.endy;
                zombie2SpriteSheet.draw(this.x, this.y);
            }
        }
    }
